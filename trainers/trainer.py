...
def train_parser():
    ...
    parser.add_argument("--stage",help="number lr stages",type=int,default=2)
    parser.add_argument("--weight_decay",help="weight decay for optimizer",type=float,default=0.0005)
    ...
    parser.add_argument("--nesterov",help="nesterov for sgd",action="store_true",default=True)
    ...
    parser.add_argument("--pre", help="whether use pre-resized 84x84 images for val and test",action="store_true",default=True)
    parser.add_argument("--no_val", help="don't use validation set, just save model at final timestep",action="store_true")
    parser.add_argument("--train_way",help="training way",type=int,default=20)
    ...
    parser.add_argument("--test_shot",nargs='+',help="number of support images per class for meta-testing during final test",type=int,default=[1,5])
    ...
    parser.add_argument("--train_transform_type",help="size transformation type during training",type=int,default=0)
...
class Train_Manager:
    ...
    def train(self,model):
        ...
        for e in tqdm(range(total_epoch)):
            ...
            iter_counter,train_acc,loss_list = train_func(model=model,
                                                optimizer=optimizer,
                                                writer=writer,
        ...
        # 在训练结束后，将 train_list 转换为 numpy 数组并保存到 .npy 文件中
        np.save('train_acc.npy', np.array(train_list))  # 保存到 'train_acc.npy' 文件

        # 将 loss_list 转换为 NumPy 数组并保存为 .npy 文件
